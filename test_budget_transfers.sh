...
# Test script for transactions between on-budget and off-budget accounts
# This script tests the fix for the bug where outgoing transactions from on-budget to off-budget accounts
# were incorrectly treated as incoming amounts.

echo "Testing transactions between on-budget and off-budget accounts..."
...
# Check account balances after transaction 2
echo "Checking on-budget account balance after transaction 2..."
ON_BUDGET_BALANCE=$(curl -s -X GET "$BASE_URL/accounts/$ON_BUDGET_ID" | jq -r '.balance')
echo "On-budget account balance: $ON_BUDGET_BALANCE (expected: 1000.0)"

echo "Checking off-budget account balance after transaction 2..."
OFF_BUDGET_BALANCE=$(curl -s -X GET "$BASE_URL/accounts/$OFF_BUDGET_ID" | jq -r '.balance')
...
# Check account balances after transaction 3
echo "Checking on-budget account balance after transaction 3..."
ON_BUDGET_BALANCE=$(curl -s -X GET "$BASE_URL/accounts/$ON_BUDGET_ID" | jq -r '.balance')
echo "On-budget account balance: $ON_BUDGET_BALANCE (expected: 1050.0)"

echo "Checking off-budget account balance after transaction 3..."
OFF_BUDGET_BALANCE=$(curl -s -X GET "$BASE_URL/accounts/$OFF_BUDGET_ID" | jq -r '.balance')
